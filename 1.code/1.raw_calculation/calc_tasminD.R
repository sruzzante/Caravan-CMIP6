...
# This script calculates the difference between the minimum and mean temperature from the raw climate data 
...
for(it in 1:nrow(datasets_wide)){
  ...
  for(it_sub in 1:10){
    ...
    targetDir = sprintf("2.data/2.working/ClimateChangeProjections/CMIP6_ESGF_uncorrected/%s/%s/tasminD",datasets_wide$source_id[it],datasets_wide$experiment_id[it])
    ...
    tasminD_mat = (tasmin_mat-tas_mat)
    
    
    tasminD_mat = (tasminD_mat-offsets["tasminD"])*fctrs["tasminD"]
    ...
    var_def <- ncvar_def(
      ...
      missval = -32768,
      longname = "Difference between daily minimum and daily mean temperature",
      prec = "short",
      shuffle = T,
      compression = 9,
    ...
    tasminD <- nc_create(paste("2.data/2.working/ClimateChangeProjections/CMIP6_ESGF_uncorrected",
                               datasets_wide$source_id[it],
                               datasets_wide$experiment_id[it],
                               "tasminD",
                               paste(
    ...
    ncvar_put(tasminD, var_def, tasminD_mat)
    
    
    ncatt_put(tasminD, "id", "gauge_id", ncatt_get(tas,varid = "id",attname = "gauge_id")$value)
    ncatt_put(tasminD, 0, "institution_id",ncatt_get(tas,0,attname = "institution_id")$value)
    ncatt_put(tasminD, 0, "institution",ncatt_get(tas,0,attname = "institution")$value)
    ncatt_put(tasminD, 0, "source_id",ncatt_get(tas,0,attname = "source_id")$value)
    ncatt_put(tasminD, 0, "source",ncatt_get(tas,0,attname = "source")$value)
    ncatt_put(tasminD, 0, "experiment_id",ncatt_get(tas,0,attname = "experiment_id")$value)
    ncatt_put(tasminD, 0, "variant_label", ncatt_get(tas,0,attname = "variant_label")$value)
    ncatt_put(tasminD, 0, "grid_label",ncatt_get(tas,0,attname = "grid_label")$value)
    ncatt_put(tasminD, 0, "grid",ncatt_get(tas,0,attname = "grid")$value)
    
    
    ncatt_put(tasminD, 0, "license",ncatt_get(tas,0,attname = "license")$value)
    ncatt_put(tasminD, 0, "Conventions",ncatt_get(tas,0,attname = "Conventions")$value)
    ncatt_put(tasminD, 0, "contact",ncatt_get(tas,0,attname = "Contact")$value)
    ncatt_put(tasminD, 0, "further_info_url",ncatt_get(tas,0,attname = "further_info_url")$value)
    
    ncatt_put(tasminD, "tasminD", "scale_factor", 1/fctrs["tasminD"])
    ncatt_put(tasminD,"tasminD", "add_offset", offsets["tasminD"])
