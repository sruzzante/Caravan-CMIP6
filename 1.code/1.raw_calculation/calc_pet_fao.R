...
# This script calculates the potential evapotranspiration from the raw climate data using the Penman-Monteith formulation (petfao56)
...
datasets=rbind(
  ...
  datasets%>%
    filter(variable_id=="uas")%>%
    mutate(variable_id="sfcWind"),
  datasets%>%
...
for(it in 1:nrow(datasets_wide)){
  ...
  for(it_sub in 1:10){
    ...
    targetDir = sprintf("2.data/2.working/ClimateChangeProjections/CMIP6_ESGF_uncorrected/%s/%s/petfao56",datasets_wide$source_id[it],datasets_wide$experiment_id[it])
    ...
    sfcWind = nc_open(datasets_wide$sfcWind[it]%>%str_replace("d.nc",sprintf("d_%d.nc",it_sub)))
    ...
    dts_sfcWind<- CFtime::CFtime(sfcWind$dim$time$units, sfcWind$dim$time$calendar, sfcWind$dim$time$vals)%>%
      CFtime::as_timestamp(format = "date")
    dts_list = list(dts_rls,dts_rss,dts_tas,dts_hurs,dts_ps,dts_sfcWind)
    ...
    mask_sfcWind = dts_sfcWind %in% dts_list[[1]]& dts_sfcWind %in% dts_list[[2]] & 
      dts_sfcWind %in% dts_list[[3]]& dts_sfcWind %in% dts_list[[5]] &
      dts_sfcWind %in% dts_list[[5]]& dts_sfcWind %in% dts_list[[6]]
    
    
    dts = dts_tas[mask_tas]
    
    tas_mat<-ncvar_get(tas,varid = "tas")[,mask_tas]
    hurs_mat<-ncvar_get(hurs,varid = "hurs")[,mask_hurs]
    ps_mat = ncvar_get(ps,varid = "ps")[,mask_ps]
    sfcWind_mat = ncvar_get(sfcWind,varid = "sfcWind")[,mask_sfcWind]
    rss_mat = ncvar_get(rss,varid = "rss")[,mask_rss]
    rls_mat = ncvar_get(rls,varid = "rls")[,mask_rls]
    
    petfao56_mat = pet_fao_func(p =ps_mat,
                                ta = tas_mat,
                                hur = hurs_mat,
                                sfcWind = sfcWind_mat,
                                rs = rss_mat,
    ...
    petfao56_mat = (petfao56_mat-offsets["petfao56"])*fctrs["petfao56"]
    ...
    petfao56 <- nc_create(paste("2.data/2.working/ClimateChangeProjections/CMIP6_ESGF_uncorrected",
                                datasets_wide$source_id[it],
                                datasets_wide$experiment_id[it],
                                "petfao56",
                                paste(
    ...
    ncvar_put(petfao56, var_def, petfao56_mat)
    
    ncatt_put(petfao56, "petfao56", "scale_factor", 1/fctrs["petfao56"])
    ncatt_put(petfao56,"petfao56", "add_offset", offsets["petfao56"])
    
    # ncvar_put(nc, gauge_var, id_vals)
    # ncvar_put(nc, gauge_var, id_char)
    
    
    ncatt_put(petfao56, "id", "gauge_id", ncatt_get(tas,varid = "id",attname = "gauge_id")$value)
    ncatt_put(petfao56, 0, "institution_id",ncatt_get(tas,0,attname = "institution_id")$value)
    ncatt_put(petfao56, 0, "institution",ncatt_get(tas,0,attname = "institution")$value)
    ncatt_put(petfao56, 0, "source_id",ncatt_get(tas,0,attname = "source_id")$value)
    ncatt_put(petfao56, 0, "source",ncatt_get(tas,0,attname = "source")$value)
    ncatt_put(petfao56, 0, "experiment_id",ncatt_get(tas,0,attname = "experiment_id")$value)
    ncatt_put(petfao56, 0, "variant_label", ncatt_get(tas,0,attname = "variant_label")$value)
    ncatt_put(petfao56, 0, "grid_label",ncatt_get(tas,0,attname = "grid_label")$value)
    ncatt_put(petfao56, 0, "grid",ncatt_get(tas,0,attname = "grid")$value)
    
    
    ncatt_put(petfao56, 0, "license",ncatt_get(tas,0,attname = "license")$value)
    ncatt_put(petfao56, 0, "Conventions",ncatt_get(tas,0,attname = "Conventions")$value)
    ncatt_put(petfao56, 0, "contact",ncatt_get(tas,0,attname = "Contact")$value)
    ncatt_put(petfao56, 0, "further_info_url",ncatt_get(tas,0,attname = "further_info_url")$value)
