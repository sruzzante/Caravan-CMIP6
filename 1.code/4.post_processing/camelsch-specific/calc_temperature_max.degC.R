...
datasets<-readRDS("2.data/2.working/CMIP6_ESGF/datasets_v2.rds")%>%
  rbind(readRDS("2.data/2.working/CMIP6_ESGF/datasets_v2.rds")%>%
          filter( variable_id == "tasmax" )%>%
          mutate(variable_id = case_when( variable_id == "tasmax" ~"tasmaxD")))
...
datasets_camels<-left_join(datasets,param_trans,by = c("variable_id" = "var_CMIP6"))%>%
  ...
  filter(var_camels %in% c('temperature_mean.degC.','temperature_max.degC.','temperature_max.degC.diff'))

datasets_camels$flNm = sprintf("/scratch/ruzzante/temp/CMIP6_ESGF_QDM_camels-ch/%s/%s/%s/%s_day_%s_%s_%s.nc",
...
for(it in 1:nrow(datasets_wide)){
  ...
  print(sprintf( "Beginning dataset row %d, %s",datasets_wide$i[it],datasets_wide$temperature_max.degC.[it]))
  # if(file.exists(datasets_wide$temperature_max.degC.[it])){
  #   print("already done, skipping")
  #   next
  # }
  
  tic()
  targetDir = sprintf("/scratch/ruzzante/temp/CMIP6_ESGF_QDM_camels-ch/%s/%s/temperature_max.degC.",datasets_wide$source_id[it],datasets_wide$experiment_id[it])
  
  if(!dir.exists(targetDir)){
    dir.create(targetDir,recursive = T)
  } 
  
  tmean.C. = nc_open(datasets_wide$temperature_mean.degC.[it])
  
  tmax.C.diff = nc_open(datasets_wide$temperature_max.degC.diff[it])
  
  
  
  tmean.C._mat<-ncvar_get(tmean.C.,varid = "temperature_mean.degC.")
  # tasC_mat = tas_mat-273.15
  tmax.C.diff_mat<-ncvar_get(tmax.C.diff,varid = "temperature_max.degC.diff")
  ...
  var_def <- ncvar_def(
    name  = "temperature_max.degC.",
    longname = "Observed daily maximum temperature",
  ...
  nc<-nc_create(datasets_wide$temperature_max.degC.[it],
                vars = list(var_def))
  
  ncatt_put(nc, "temperature_max.degC.",  "scale_factor", 1/100)
  ncatt_put(nc,"temperature_max.degC.", "add_offset", 0)
