...
datasets_camels<-left_join(datasets,param_trans,by = c("variable_id" = "var_CMIP6"))%>%
  ...
  filter(var_camels %in% c('temperature_mean.degC.','temperature_min.degC.','temperature_min.degC.diff'))
...
for(it in 1:nrow(datasets_wide)){
  ...
  print(sprintf( "Beginning dataset row %d, %s",datasets_wide$i[it],datasets_wide$temperature_min.degC.[it]))
  
  # if(file.exists(datasets_wide$temperature_min.degC.[it])){
  #   print("already done, skipping")
  #   next
  # }
  tic()
  targetDir = sprintf("/scratch/ruzzante/temp/CMIP6_ESGF_QDM_camels-ch/%s/%s/temperature_min.degC.",datasets_wide$source_id[it],datasets_wide$experiment_id[it])
  ...
  tmin.C.diff = nc_open(datasets_wide$temperature_min.degC.diff[it])
  
  
  
  tmean.C._mat<-ncvar_get(tmean.C.,varid = "temperature_mean.degC.")
  # tasC_mat = tas_mat-273.15
  tmin.C.diff_mat<-ncvar_get(tmin.C.diff,varid = "temperature_min.degC.diff")
  ...
  var_def <- ncvar_def(
    name  = "temperature_min.degC.",
    longname = "Observed daily minimum temperature",
  ...
  nc<-nc_create(datasets_wide$temperature_min.degC.[it],
                vars = list(var_def))
  
  ncatt_put(nc, "temperature_min.degC.",  "scale_factor", 1/100)
  ncatt_put(nc,"temperature_min.degC.", "add_offset", 0)
