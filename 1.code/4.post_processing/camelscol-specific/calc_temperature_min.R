...
params<-read.csv("2.data/2.working/histMetData/camels-col/param_trans_camels_col.csv")
...
datasets_camels<-left_join(datasets,params,by = c("variable_id" = "var_CMIP6"))%>%
  ...
  filter(var_camels %in% c('t_minDiff','t_mean','t_min'))

datasets_camels$flNm = sprintf("/scratch/ruzzante/temp/CMIP6_ESGF_QDM_camels-col/%s/%s/%s/%s_day_%s_%s_%s.nc",
...
for(it in 1:nrow(datasets_wide)){
  ...
  print(sprintf( "Beginning dataset row %d, %s",datasets_wide$i[it],datasets_wide$t_min[it]))
  
  # if(file.exists(datasets_wide$t_min[it])){
  #   print("already done, skipping")
  #   next
  # }
  tic()
  targetDir = sprintf("/scratch/ruzzante/temp/CMIP6_ESGF_QDM_camels-col/%s/%s/t_min",datasets_wide$source_id[it],datasets_wide$experiment_id[it])
  
  if(!dir.exists(targetDir)){
    dir.create(targetDir,recursive = T)
  } 
  
  tmean.C. = nc_open(datasets_wide$t_mean[it])
  
  tmin.C.diff = nc_open(datasets_wide$t_minDiff[it])
  
  
  
  tmean.C._mat<-ncvar_get(tmean.C.,varid = "t_mean")
  # tasC_mat = tas_mat-273.15
  tmin.C.diff_mat<-ncvar_get(tmin.C.diff,varid = "t_minDiff")
  ...
  var_def <- ncvar_def(
    name  = "t_min",
    longname = "Daily maximum temperature of catchment",
  ...
  nc<-nc_create(datasets_wide$t_min[it],
                vars = list(var_def))
  
  ncatt_put(nc, "t_min",  "scale_factor", 1/100)
  ncatt_put(nc,"t_min", "add_offset", 0)
