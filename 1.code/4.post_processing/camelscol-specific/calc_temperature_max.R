...
datasets_camels<-left_join(datasets,params,by = c("variable_id" = "var_CMIP6"))%>%
  ...
  filter(var_camels %in% c('t_maxDiff','t_mean','t_max'))
...
for(it in 1:nrow(datasets_wide)){
  ...
  print(sprintf( "Beginning dataset row %d, %s",datasets_wide$i[it],datasets_wide$t_max[it]))
  # if(file.exists(datasets_wide$t_max[it])){
  #   print("already done, skipping")
  #   next
  # }
  
  tic()
  targetDir = sprintf("/scratch/ruzzante/temp/CMIP6_ESGF_QDM_camels-col/%s/%s/t_max",datasets_wide$source_id[it],datasets_wide$experiment_id[it])
  ...
  tmax.C.diff = nc_open(datasets_wide$t_maxDiff[it])
  
  
  
  tmean.C._mat<-ncvar_get(tmean.C.,varid = "t_mean")
  # tasC_mat = tas_mat-273.15
  tmax.C.diff_mat<-ncvar_get(tmax.C.diff,varid = "t_maxDiff")
  ...
  nc<-nc_create(datasets_wide$t_max[it],
                vars = list(var_def))
  
  ncatt_put(nc, "t_max",  "scale_factor", 1/100)
  ncatt_put(nc,"t_max", "add_offset", 0)
