...
datasets_camels<-left_join(datasets,params,by = c("variable_id" = "var_CMIP6"))%>%
  ...
  filter(var_camels %in% c('tmin_SILODiff','tmin_SILO', "tmax_SILO"))
...
for(it in 1:nrow(datasets_wide)){
  ...
  print(sprintf( "Beginning dataset row %d, %s",datasets_wide$i[it],datasets_wide$tmin_SILO[it]))
  
  if(file.exists(datasets_wide$tmin_SILO[it])){
    print("already done, skipping")
    next
  }
  tic()
  targetDir = sprintf("/scratch/ruzzante/temp/CMIP6_ESGF_QDM_camels-aus/%s/%s/tmin_SILO",datasets_wide$source_id[it],datasets_wide$experiment_id[it])
  ...
  tmin.C.diff = nc_open(datasets_wide$tmin_SILODiff[it])
  
  
  
  tmax.C._mat<-ncvar_get(tmax.C.,varid = "tmax_SILO")
  # tasC_mat = tas_mat-273.15
  tmin.C.diff_mat<-ncvar_get(tmin.C.diff,varid = "tmin_SILODiff")
  ...
  var_def <- ncvar_def(
    name  = "tmin_SILO",
    longname = "Daily minimum temperature from Scientific Information for Land Owners (Jeffrey et al., 2001)",
  ...
  nc<-nc_create(datasets_wide$tmin_SILO[it],
                vars = list(var_def))
  
  ncatt_put(nc, "tmin_SILO",  "scale_factor", 1/100)
  ncatt_put(nc,"tmin_SILO", "add_offset", 0)
