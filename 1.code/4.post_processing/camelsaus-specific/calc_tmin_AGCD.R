...
datasets_camels<-left_join(datasets,params,by = c("variable_id" = "var_CMIP6"))%>%
  ...
  filter(var_camels %in% c('tmin_AGCDDiff','tmin_AGCD', "tmax_AGCD"))
...
for(it in 1:nrow(datasets_wide)){
  ...
  print(sprintf( "Beginning dataset row %d, %s",datasets_wide$i[it],datasets_wide$tmin_AGCD[it]))
  
  if(file.exists(datasets_wide$tmin_AGCD[it])){
    print("already done, skipping")
    next
  }
  tic()
  targetDir = sprintf("/scratch/ruzzante/temp/CMIP6_ESGF_QDM_camels-aus/%s/%s/tmin_AGCD",datasets_wide$source_id[it],datasets_wide$experiment_id[it])
  
  if(!dir.exists(targetDir)){
    dir.create(targetDir,recursive = T)
  } 
  
  tmax.C. = nc_open(datasets_wide$tmax_AGCD[it])
  
  tmin.C.diff = nc_open(datasets_wide$tmin_AGCDDiff[it])
  
  
  
  tmax.C._mat<-ncvar_get(tmax.C.,varid = "tmax_AGCD")
  # tasC_mat = tas_mat-273.15
  tmin.C.diff_mat<-ncvar_get(tmin.C.diff,varid = "tmin_AGCDDiff")
  ...
  var_def <- ncvar_def(
    name  = "tmin_AGCD",
    longname = "Daily minimum temperature from Australian Gridded Climate Data v1.0.1 (Evans et al., 2020)",
  ...
  nc<-nc_create(datasets_wide$tmin_AGCD[it],
                vars = list(var_def))
  
  ncatt_put(nc, "tmin_AGCD",  "scale_factor", 1/100)
  ncatt_put(nc,"tmin_AGCD", "add_offset", 0)
