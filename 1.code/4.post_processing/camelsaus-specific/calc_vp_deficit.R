...
params<-read.csv("2.data/2.working/histMetData/camels-aus/param_trans_camels_aus.csv")%>%
  # filter(final_var)%>%
...
datasets_camels<-left_join(datasets,params,by = c("variable_id" = "var_CMIP6"))%>%
  ...
  filter(var_camels %in% c('vp_SILO','vp_deficit_SILO','tmin_SILO', "tmax_SILO"))

datasets_camels$flNm = sprintf("/scratch/ruzzante/temp/CMIP6_ESGF_QDM_camels-aus/%s/%s/%s/%s_day_%s_%s_%s.nc",
...
for(it in 1:nrow(datasets_wide)){
  ...
  print(sprintf( "Beginning dataset row %d, %s",datasets_wide$i[it],datasets_wide$vp_deficit_SILO[it]))
  
  if(file.exists(datasets_wide$vp_deficit_SILO[it])){
    print("already done, skipping")
    next
  }
  tic()
  targetDir = sprintf("/scratch/ruzzante/temp/CMIP6_ESGF_QDM_camels-aus/%s/%s/vp_deficit_SILO",datasets_wide$source_id[it],datasets_wide$experiment_id[it])
  
  if(!dir.exists(targetDir)){
    dir.create(targetDir,recursive = T)
  } 
  
  tmax.C. = nc_open(datasets_wide$tmax_SILO[it])
  
  tmin.C. = nc_open(datasets_wide$tmin_SILO[it])
  
  vp_SILO = nc_open(datasets_wide$vp_SILO[it])
  ...
  vp_SILO_mat = ncvar_get(vp_SILO,varid = "vp_SILO")
  
  svp_Pa = vp_func(100,tmean.C._mat+273.15)
  
  vp_deficit_SILO_mat = svp_Pa / 100 - vp_SILO_mat
  
  vp_deficit_SILO_mat = (vp_deficit_SILO_mat-0)*100 # hard-code offset and scale here, same as tmax
  ...
  var_def <- ncvar_def(
    name  = "vp_deficit_SILO",
    longname = "Vapour pressure deficit from Scientific Information for Land Owners (Jeffrey et al., 2001)",
  ...
  nc<-nc_create(datasets_wide$vp_deficit_SILO[it],
                vars = list(var_def))
  
  ncatt_put(nc, "vp_deficit_SILO",  "scale_factor", 1/100)
  ncatt_put(nc,"vp_deficit_SILO", "add_offset", 0)
  
  
  ncvar_put(nc, var_def, vp_deficit_SILO_mat)
