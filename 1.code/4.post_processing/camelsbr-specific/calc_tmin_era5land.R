...
datasets_camels<-left_join(datasets,params,by = c("variable_id" = "var_CMIP6"))%>%
  ...
  filter(var_camels %in% c('tmin_era5land','tmean_era5land','tmin_era5landDiff'))
...
for(it in 1:nrow(datasets_wide)){
  ...
  print(sprintf( "Beginning dataset row %d, %s",datasets_wide$i[it],datasets_wide$tmin_era5land[it]))
  
  # if(file.exists(datasets_wide$tmin_era5land[it])){
  #   print("already done, skipping")
  #   next
  # }
  tic()
  targetDir = sprintf("/scratch/ruzzante/temp/CMIP6_ESGF_QDM_camels-br/%s/%s/tmin_era5land",datasets_wide$source_id[it],datasets_wide$experiment_id[it])
  ...
  tmin.C.diff = nc_open(datasets_wide$tmin_era5landDiff[it])
  
  
  
  tmean.C._mat<-ncvar_get(tmean.C.,varid = "tmean_era5land")
  # tasC_mat = tas_mat-273.15
  tmin.C.diff_mat<-ncvar_get(tmin.C.diff,varid = "tmin_era5landDiff")
  ...
  var_def <- ncvar_def(
    name  = "tmin_era5land",
    longname = "Daily minimum temperature from ERA5LAND",
  ...
  nc<-nc_create(datasets_wide$tmin_era5land[it],
                vars = list(var_def))
  
  ncatt_put(nc, "tmin_era5land",  "scale_factor", 1/100)
  ncatt_put(nc,"tmin_era5land", "add_offset", 0)
