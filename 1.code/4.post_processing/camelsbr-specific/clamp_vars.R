...
for(it in 1:nrow(datasets_camels)){
  ...
  flNm = sprintf("/scratch/ruzzante/temp/CMIP6_ESGF_QDM_camels-br/%s/%s/%s/%s_day_%s_%s_%s.nc",
  ...
  if(! ncatt_get(nc,varid = datasets_camels$var_camels[it],attname = "add_offset")$hasatt|
    ...
    }else if(datasets_camels$max_theo[it]%in% c("tmean_era5land","tmax_cpc","tmax_brdwgd")){
      
      nc_temp =  sprintf("/scratch/ruzzante/temp/CMIP6_ESGF_QDM_camels-br/%s/%s/%s/%s_day_%s_%s_%s.nc",
                         datasets_camels$source_id[it],
                         datasets_camels$experiment_id[it],
    ...
    }else if(datasets_camels$min_theo[it]%in% c("tmean_era5land","tmax_cpc","tmax_brdwgd")){
