...
datasets_camels<-left_join(datasets,params,by = c("variable_id" = "var_CMIP6"))%>%
  ...
  filter(var_camels %in% c('tmin_brdwgdDiff','tmin_brdwgd', "tmax_brdwgd"))
...
for(it in 1:nrow(datasets_wide)){
  ...
  print(sprintf( "Beginning dataset row %d, %s",datasets_wide$i[it],datasets_wide$tmin_brdwgd[it]))
  
  # if(file.exists(datasets_wide$tmin_brdwgd[it])){
  #   print("already done, skipping")
  #   next
  # }
  tic()
  targetDir = sprintf("/scratch/ruzzante/temp/CMIP6_ESGF_QDM_camels-br/%s/%s/tmin_brdwgd",datasets_wide$source_id[it],datasets_wide$experiment_id[it])
  
  if(!dir.exists(targetDir)){
    dir.create(targetDir,recursive = T)
  } 
  
  tmax.C. = nc_open(datasets_wide$tmax_brdwgd[it])
  
  tmin.C.diff = nc_open(datasets_wide$tmin_brdwgdDiff[it])
  
  
  
  tmax.C._mat<-ncvar_get(tmax.C.,varid = "tmax_brdwgd")
  # tasC_mat = tas_mat-273.15
  tmin.C.diff_mat<-ncvar_get(tmin.C.diff,varid = "tmin_brdwgdDiff")
  ...
  var_def <- ncvar_def(
    name  = "tmin_brdwgd",
    longname = "Daily minimum temperature from Brazilian Daily Weather Gridded Data(BR-DWGD) v3.2.3",
  ...
  nc<-nc_create(datasets_wide$tmin_brdwgd[it],
                vars = list(var_def))
  
  ncatt_put(nc, "tmin_brdwgd",  "scale_factor", 1/100)
  ncatt_put(nc,"tmin_brdwgd", "add_offset", 0)
