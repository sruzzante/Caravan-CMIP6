...
datasets_camels<-left_join(datasets,params,by = c("variable_id" = "var_CMIP6"))%>%
  ...
  filter(var_camels %in% c('tmax_era5land','tmean_era5land','tmax_era5landDiff'))
...
for(it in 1:nrow(datasets_wide)){
  ...
  print(sprintf( "Beginning dataset row %d, %s",datasets_wide$i[it],datasets_wide$tmax_era5land[it]))
  # if(file.exists(datasets_wide$tmax_era5land[it])){
  #   print("already done, skipping")
  #   next
  # }
  
  tic()
  targetDir = sprintf("/scratch/ruzzante/temp/CMIP6_ESGF_QDM_camels-br/%s/%s/tmax_era5land",datasets_wide$source_id[it],datasets_wide$experiment_id[it])
  
  if(!dir.exists(targetDir)){
    dir.create(targetDir,recursive = T)
  } 
  
  tmean.C. = nc_open(datasets_wide$tmean_era5land[it])
  
  tmax.C.diff = nc_open(datasets_wide$tmax_era5landDiff[it])
  
  
  
  tmean.C._mat<-ncvar_get(tmean.C.,varid = "tmean_era5land")
  # tasC_mat = tas_mat-273.15
  tmax.C.diff_mat<-ncvar_get(tmax.C.diff,varid = "tmax_era5landDiff")
  ...
  var_def <- ncvar_def(
    name  = "tmax_era5land",
    longname = "Daily maximum temperature from ERA5-LAND",
  ...
  nc<-nc_create(datasets_wide$tmax_era5land[it],
                vars = list(var_def))
  
  ncatt_put(nc, "tmax_era5land",  "scale_factor", 1/100)
  ncatt_put(nc,"tmax_era5land", "add_offset", 0)
