...
param_trans<-read.csv("2.data/2.working/histMetData/camels-gb-v2/param_trans_camels-gb.csv")%>%
  filter(final_var == T)
...
datasets<-readRDS("2.data/2.working/CMIP6_ESGF/datasets_v2.rds")%>%
  mutate(variable_id = case_when(variable_id == "hurs"~ "petfao56",
                                 
                                 !variable_id %in% c("hurs") ~variable_id))
...
for(it in 1:nrow(datasets_camels)){
  ...
  flNm = sprintf("/scratch/ruzzante/temp/CMIP6_ESGF_QDM_camels-gb/%s/%s/%s/%s_day_%s_%s_%s.nc",
