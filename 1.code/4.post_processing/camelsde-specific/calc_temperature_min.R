...
params<-read.csv("2.data/2.working/histMetData/camels-de/param_trans_camels_de.csv")
...
datasets_camels<-left_join(datasets,params,by = c("variable_id" = "var_CMIP6"))%>%
  ...
  filter(var_camels %in% c('temperature_minDiff','temperature_mean','temperature_min'))

datasets_camels$flNm = sprintf("/scratch/ruzzante/temp/CMIP6_ESGF_QDM_camels-de/%s/%s/%s/%s_day_%s_%s_%s.nc",
...
for(it in 1:nrow(datasets_wide)){
  ...
  print(sprintf( "Beginning dataset row %d, %s",datasets_wide$i[it],datasets_wide$temperature_min[it]))
  
  # if(file.exists(datasets_wide$temperature_min[it])){
  #   print("already done, skipping")
  #   next
  # }
  tic()
  targetDir = sprintf("/scratch/ruzzante/temp/CMIP6_ESGF_QDM_camels-de/%s/%s/temperature_min",datasets_wide$source_id[it],datasets_wide$experiment_id[it])
  
  if(!dir.exists(targetDir)){
    dir.create(targetDir,recursive = T)
  } 
  
  tmean.C. = nc_open(datasets_wide$temperature_mean[it])
  
  tmin.C.diff = nc_open(datasets_wide$temperature_minDiff[it])
  
  
  
  tmean.C._mat<-ncvar_get(tmean.C.,varid = "temperature_mean")
  # tasC_mat = tas_mat-273.15
  tmin.C.diff_mat<-ncvar_get(tmin.C.diff,varid = "temperature_minDiff")
  ...
  var_def <- ncvar_def(
    ...
    longname = "Observed interpolated spatial mean daily minimum temperatures",
  ...
  nc<-nc_create(datasets_wide$temperature_min[it],
                vars = list(var_def))
  
  ncatt_put(nc, "temperature_min",  "scale_factor", 1/100)
  ncatt_put(nc,"temperature_min", "add_offset", 0)
