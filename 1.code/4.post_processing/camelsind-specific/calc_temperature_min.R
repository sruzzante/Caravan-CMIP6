...
datasets_camels<-left_join(datasets,params,by = c("variable_id" = "var_CMIP6"))%>%
  ...
  filter(var_camels %in% c('tmin.C.Diff','tavg.C.','tmin.C.'))
...
for(it in 1:nrow(datasets_wide)){
  ...
  print(sprintf( "Beginning dataset row %d, %s",datasets_wide$i[it],datasets_wide$tmin.C.[it]))
  ...
  # }
  tic()
  targetDir = sprintf("/scratch/ruzzante/temp/CMIP6_ESGF_QDM_camels-ind/%s/%s/tmin.C.",datasets_wide$source_id[it],datasets_wide$experiment_id[it])
  ...
  nc<-nc_create(datasets_wide$tmin.C.[it],
                vars = list(var_def))
