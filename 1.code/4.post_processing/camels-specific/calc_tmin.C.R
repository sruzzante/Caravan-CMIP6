...
datasets_camels<-left_join(datasets,param_trans,by = c("variable_id" = "var_CMIP6"))%>%
  mutate(var_CMIP6 = variable_id)%>%
  filter(!is.na(var_camels))%>%
  cross_join(data.frame(bias_correct_target = c("daymet")))
...
datasets_wide<-datasets_wide%>%
  mutate(flNm = sprintf("/scratch/ruzzante/temp/CMIP6_ESGF_QDM_camels/%s/%s/tmin.C./tmin.C._day_%s_%s_%s_uncorrected_%s.nc",
                        source_id,
                        experiment_id,
                        source_id,
...
for(it in 1:nrow(datasets_wide)){
  ...
  if(file.exists(datasets_wide$flNm[it])){print("already done, skipping"); next}
  
  targetDir = sprintf("/scratch/ruzzante/temp/CMIP6_ESGF_QDM_camels/%s/%s/tmin.C.",datasets_wide$source_id[it],datasets_wide$experiment_id[it])
  
  if(!dir.exists(targetDir)){
    dir.create(targetDir,recursive = T)
  } 
  
  tmax.C. = nc_open(datasets_wide$tmax.C.[it])
  
  if(datasets_wide$bias_correct_target[it] %in% "daymet"){
    tmin.C.diff = nc_open(datasets_wide$tmin.C.diff[it])
    tmin.C.diff_mat<-ncvar_get(tmin.C.diff,varid = "tmin.C.diff")
  ...
  missing_value = -32768
  lngNm = NULL
  if(datasets_wide$bias_correct_target[it]=="daymet"){
    lngNm = "2m minimum air temperature"
  }else if(datasets_wide$bias_correct_target[it]=="maurer"){
    lngNm = "2m mean air temperature (Maurer forcing tmax and tmin are actually mean temperature in CAMELS)"
  }else if(datasets_wide$bias_correct_target[it]=="nldas"){
    lngNm = "2m mean air temperature (NLDAS forcing tmax and tmin are actually mean temperature in CAMELS)"
  }
  ...
  ncatt_put(nc, "tmin.C.",  "scale_factor", 1/100)
  ncatt_put(nc,"tmin.C.", "add_offset", 0)
