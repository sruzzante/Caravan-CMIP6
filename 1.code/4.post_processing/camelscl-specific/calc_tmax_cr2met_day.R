...
for(it in 1:nrow(datasets_wide)){
  ...
  print(sprintf( "Beginning dataset row %d, %s",datasets_wide$i[it],datasets_wide$tmax_cr2met_C_day[it]))
  if(file.exists(datasets_wide$tmax_cr2met_C_day[it])){
    print("already done, skipping")
    next
  }
  
  tic()
  targetDir = sprintf("/scratch/ruzzante/temp/CMIP6_ESGF_QDM_camels-cl/%s/%s/tmax_cr2met_C_day",datasets_wide$source_id[it],datasets_wide$experiment_id[it])
  ...
  tmax.C.diff = nc_open(datasets_wide$tmax_cr2met_C_day_diff[it])
  
  
  
  tmean.C._mat<-ncvar_get(tmean.C.,varid = "tmean_cr2met_C_day")
  # tasC_mat = tas_mat-273.15
  tmax.C.diff_mat<-ncvar_get(tmax.C.diff,varid = "tmax_cr2met_C_day_diff")
  ...
  var_def <- ncvar_def(
    name  = "tmax_cr2met_C_day",
    longname = "Daily maximum temperature from CR2METv1.3",
  ...
  nc<-nc_create(datasets_wide$tmax_cr2met_C_day[it],
                vars = list(var_def))
  
  ncatt_put(nc, "tmax_cr2met_C_day",  "scale_factor", 1/100)
  ncatt_put(nc,"tmax_cr2met_C_day", "add_offset", 0)
