...
datasets<-readRDS("2.data/2.working/CMIP6_ESGF/datasets_v2.rds")%>%
  mutate(variable_id = case_when(variable_id == "rlds" ~ "petharg" ,# pick a variable not used by camels-cl
                                 !variable_id %in% c("rlds") ~variable_id))
...
for(it in 1:nrow(datasets_camels)){
  ...
  flNm = sprintf("/scratch/ruzzante/temp/CMIP6_ESGF_QDM_camels-cl/%s/%s/%s/%s_day_%s_%s_%s.nc",
  ...
  if(! ncatt_get(nc,varid = datasets_camels$var_camels[it],attname = "add_offset")$hasatt|
    ...
    }else if(datasets_camels$max_theo[it]%in% c("tmean_cr2met_C_day")){
      
      nc_temp =  sprintf("/scratch/ruzzante/temp/CMIP6_ESGF_QDM_camels-cl/%s/%s/%s/%s_day_%s_%s_%s.nc",
                         datasets_camels$source_id[it],
                         datasets_camels$experiment_id[it],
    ...
    }else if(datasets_camels$min_theo[it]%in% c("tmean_cr2met_C_day")){
