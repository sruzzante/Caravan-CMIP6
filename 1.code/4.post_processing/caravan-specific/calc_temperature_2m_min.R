...
params<-read.csv("2.data/2.working/histMetData/param_trans_ERA5-Land.csv")
...
datasets = rbind(datasets,
                 ...
                 datasets%>%filter(variable_id == "hurs")%>%mutate(variable_id="tdps")
)
...
datasets_camels<-left_join(datasets,params,by = c("variable_id" = "var_CMIP6"))%>%
  ...
  filter(!is.na(var_ERA5_Land))%>%
  filter(var_ERA5_Land %in% c('temperature_2m_min_diff','temperature_2m_mean','temperature_2m_min'))%>%
  cross_join(data.frame(sub_watershed = 1:10))

datasets_camels$flNm = sprintf("2.data/2.working/ClimateChangeProjections/CMIP6_ESGF_QDM/%s/%s/%s/%s_day_%s_%s_%s_uncorrected_%d.nc",
                               datasets_camels$source_id,
                               datasets_camels$experiment_id,
                               datasets_camels$var_ERA5_Land,
                               datasets_camels$var_ERA5_Land,
                               datasets_camels$source_id,
                               
                               datasets_camels$experiment_id,
                               datasets_camels$variant_label,
                               datasets_camels$sub_watershed)


datasets_wide = datasets_camels%>%
  pivot_wider(names_from = var_ERA5_Land,values_from = flNm,
              id_cols = c(source_id,experiment_id,variant_label,sub_watershed))
...
for(it in 1:nrow(datasets_wide)){
  ...
  print(sprintf( "Beginning dataset row %d, %s",datasets_wide$i[it],datasets_wide$temperature_2m_min[it]))
  if(file.exists(datasets_wide$temperature_2m_min[it])){
    print("already done, skipping")
    next
  }
  
  tic()
  targetDir = sprintf("2.data/2.working/ClimateChangeProjections/CMIP6_ESGF_QDM/%s/%s/temperature_2m_min",datasets_wide$source_id[it],datasets_wide$experiment_id[it])
  
  if(!dir.exists(targetDir)){
    dir.create(targetDir,recursive = T)
  } 
  
  tmean.C. = nc_open(datasets_wide$temperature_2m_mean[it])
  
  tmin.C.diff = nc_open(datasets_wide$temperature_2m_min_diff[it])
  
  
  
  tmean.C._mat<-ncvar_get(tmean.C.,varid = "temperature_2m_mean")
  # tasC_mat = tas_mat-273.15
  tmin.C.diff_mat<-ncvar_get(tmin.C.diff,varid = "temperature_2m_min_diff")
  ...
  var_def <- ncvar_def(
    ...
    # longname = "Observed interpolated spatial mean daily maximum temperatures",
    # units = sprintf("%s * %0.1f",unique(units(xRast)),fctrs[datasets$variable_id[it]]),
    units ="2m minimum air temperature [°C]",
  ...
  nc<-nc_create(datasets_wide$temperature_2m_min[it],
                vars = list(var_def))
  
  ncatt_put(nc, "temperature_2m_min",  "scale_factor", 1/100)
  ncatt_put(nc,"temperature_2m_min", "add_offset", 0)
