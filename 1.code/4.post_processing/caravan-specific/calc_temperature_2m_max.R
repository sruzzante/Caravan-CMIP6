...
datasets_camels<-left_join(datasets,params,by = c("variable_id" = "var_CMIP6"))%>%
  ...
  filter(var_ERA5_Land %in% c('temperature_2m_max_diff','temperature_2m_mean','temperature_2m_max'))%>%
...
for(it in 1:nrow(datasets_wide)){
  ...
  print(sprintf( "Beginning dataset row %d, %s",datasets_wide$i[it],datasets_wide$temperature_2m_max[it]))
  if(file.exists(datasets_wide$temperature_2m_max[it])){
    print("already done, skipping")
    next
  }
  
  tic()
  targetDir = sprintf("2.data/2.working/ClimateChangeProjections/CMIP6_ESGF_QDM/%s/%s/temperature_2m_max",datasets_wide$source_id[it],datasets_wide$experiment_id[it])
  ...
  tmax.C.diff = nc_open(datasets_wide$temperature_2m_max_diff[it])
  
  
  
  tmean.C._mat<-ncvar_get(tmean.C.,varid = "temperature_2m_mean")
  # tasC_mat = tas_mat-273.15
  tmax.C.diff_mat<-ncvar_get(tmax.C.diff,varid = "temperature_2m_max_diff")
  ...
  var_def <- ncvar_def(
    ...
    units ="2m max air temperature [°C]",
  ...
  nc<-nc_create(datasets_wide$temperature_2m_max[it],
                vars = list(var_def))
  
  ncatt_put(nc, "temperature_2m_max",  "scale_factor", 1/100)
  ncatt_put(nc,"temperature_2m_max", "add_offset", 0)
